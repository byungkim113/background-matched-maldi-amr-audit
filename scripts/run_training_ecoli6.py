#!/usr/bin/env python3
"""Run the E. coli mechanism6 Mega_Model experiment.

This wrapper pins the expanded E. coli panel used for the transfer/background
argument: ciprofloxacin, norfloxacin, amox-clav, ceftriaxone, ceftazidime, and
cefepime.
"""
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Run Mega_Model ecoli_mechanism6 training/evaluation.")
    p.add_argument("--mega-model", type=Path, default=DEFAULT_MEGA)
    p.add_argument("--data-root", type=Path, required=True)
    p.add_argument("--output-dir", type=Path, default=DEFAULT_OUTPUT)
    p.add_argument("--experiment", default="exp_ecoli_mechanism6_drugid_mae30")
    p.add_argument("--mae-epochs", type=int, default=30)
    p.add_argument("--early-stop", choices=["macro", "primary"], default="primary")
    ...
    p.add_argument("--bn-adapt", action="store_true", help="Enable external-site BN adaptation.")
    p.add_argument(
        "--prevalence-shift",
        choices=["capped", "none"],
        default="none",
        help="Default none keeps external thresholds fixed for clean transfer testing.",
    )
    return p
...
def main() -> None:
    ...
    cmd = [
        ...
        "--prevalence-shift", args.prevalence_shift,
    ]
