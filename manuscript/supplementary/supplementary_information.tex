...
Figure 3 & \texttt{source\_data\_fig3\_model\_family\_replication.csv} & CNN, LGBM and Weis/Borgwardt-style compatibility audit rows. \\
Figure 4 & \texttt{source\_data\_fig4\_cross\_resistance\_edges\_all\_sites.csv} & Cross-resistance edges used for the phi heatmap. \\
Figure 5a & \texttt{source\_data\_fig5a\_public\_wgs\_maldi\_auc.csv} & Public WGS-linked MALDI AUC values. \\
Figure 5b & \texttt{source\_data\_fig5b\_proteomic\_biomarker\_enrichment.csv} & ST131 biomarker enrichment values. \\
Figure 6 & \texttt{source\_data\_fig6\_falsification\_controls.csv} & Background-burden and within-background shuffle controls. \\
Figure 7 & \texttt{source\_data\_fig7\_deployment\_decision\_flow.csv} & Deployment decision rule summaries. \\
\bottomrule
\end{tabular}
\end{table}

\section*{Supplementary Note 4: Weis/Borgwardt compatibility versus replication}

The Weis/Borgwardt-style rows are included to show that the audit can consume predictions from a published MALDI-AMR workflow. They should not be described as an exact Weis \textit{et al.} replication unless a separate raw-metric parity check against the upstream stored result JSONs passes for the same model family, seed, split policy and preprocessing. The repository includes a published-code exporter and a raw-metric comparator for that purpose.

\section*{Supplementary Note 5: What would strengthen the biological claim}
